...
/**
 *Takes a reference to a stringstream that contains our sentence and a reference to grammar map.
 * This function manipulates the random sentence and put the sequence in stringstream. Recursive call
 * is terminated when there is no nonterminal reamining.
 */

void generateSentence(stringstream& sentence, map<string,Definition>& grammar, string start){
    
    Definition def = grammar[start];
    Production randomProduction = def.getRandomProduction();
    
    for (Production::iterator itr =  randomProduction.begin(); itr<randomProduction.end(); itr++) {
        
        string str = *itr;
        if (str[0] == '<') {
            generateSentence(sentence, grammar,str);
        }else{
            sentence << str + ' ';
        }
...
int main(int argc, char *argv[])
{
    ...
    generateSentence(sentence,grammar,"<start>");
